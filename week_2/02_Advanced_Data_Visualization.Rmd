...
vaccines_safe %>%
  filter(region_group != "OTHER") %>% 
  mutate(region_group = fct_reorder(region_group, safe_agree, .fun = 'median')) %>%
  ggplot(aes(x = safe_agree, y = factor(region_group), fill = factor(region_group))) +
  stat_density_ridges(alpha = 0.8,quantile_lines = TRUE, quantiles = 2,
  ...
  labs(x = "% of people who think vaccines are safe", y = NULL,
       ...
       caption = "*each tick is one country\nSource: World Global Monitor (2018)") +
  scale_fill_paletteer_d("rcartocolor::Vivid") +
  scale_x_continuous(labels = scales::percent, limits = c(0,1)) +
  scale_y_discrete(expand = expansion(0,0)) +
  geom_label(data = region_labels, aes(x = 0,label = region_group), nudge_x = 0.25, nudge_y = 0.5, color = "white",
             size = 8,
             family = "Karla",
             fontface = "bold") +
  theme_ridges() +
  theme(
      ...
      axis.text.x = element_text(family = "Karla", size = 18),
      axis.title.x = element_text(family = "Karla", size = 22),
      axis.text.y = element_blank(),
      plot.title = element_text(family = "Karla", size = 30),
      plot.subtitle = element_text(family = "Karla", size = 36),
